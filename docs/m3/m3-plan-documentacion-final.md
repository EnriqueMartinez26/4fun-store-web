...
Este documento constituye el plan maestro del **Hito M3** para la consolidación de la documentación técnica y académica del proyecto **4Fun Store**.
...
### ¿Por qué el stock real son las `DigitalKey`?
...
- **Responsabilidad única (SRP)**:
  - El **pago** certifica que los fondos del comprador han salido de su cuenta y están en poder de la plataforma.
...
- `Order.status` representa el **ciclo operativo y logístico** de la orden (por ejemplo, `PENDING`, `PROCESSING`, `SHIPPED`, `DELIVERED`, `CANCELLED`).
- `isPaid` indica si el pago fue confirmado.
- `Transaction.status` representa el **estado financiero y de custodia del dinero** (por ejemplo, `PENDING_APPROVAL`, `FUNDS_RELEASED`, `REJECTED`).
- _Caso Académico_: Una orden puede permanecer en estado logístico `PENDING` aunque su pago haya sido confirmado (`isPaid = true`) y los fondos de la transacción hayan sido liberados al vendedor (`FUNDS_RELEASED`).
...
4. **Minuto 4:30 - 6:30 | Asignación de Inventario y Escrow**:
   - Mostrar cómo se asignan de forma automática las `DigitalKey` (cambiando su estado a `SOLD` y asociándose a la orden).
