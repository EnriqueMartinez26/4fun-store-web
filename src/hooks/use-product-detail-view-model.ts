...
export function useProductDetailViewModel(game: Game) {
  ...
  const getEmbedUrl = (url: string) => {
    ...
    if (match && match[1]) {
      return {
        type: 'youtube',
        src: `https://www.youtube-nocookie.com/embed/${match[1]}?autoplay=1&mute=1&loop=1&playlist=${match[1]}&rel=0&modestbranding=1&playsinline=1`,
      };
    }
  ...
  const editUrl = isAdmin ? `/admin/products/${game.id}` : `/seller/products/${game.id}`;
  ...
  const handleAddToCart = () => {
    if (isOutOfStock) {
      toast({
        ...
        description: 'El activo se encuentra sin unidades en circulación.',
      });
      return;
    }
    addToCart(game);
