    ...
    const hydrate = async () => {
      try {
        ...
        if (product) {
          ...
          const devIsPreset = (DEVELOPERS as readonly string[]).includes(
            rawProduct.developer as string
          );
          ...
          form.reset({
            ...
            type: (rawProduct.type as 'Digital') || 'Digital',
            developer: rawProduct.developer || '',
            specPreset: rawProduct.specPreset
              ? rawProduct.specPreset.charAt(0).toUpperCase() +
                rawProduct.specPreset.slice(1).toLowerCase()
              : 'Mid',
