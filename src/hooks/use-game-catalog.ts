...
const FALLBACK_MAX_PRICE_ARS = 300000;

const normalizeMaxPrice = (value: number) => {
  if (!Number.isFinite(value) || value <= 0) return FALLBACK_MAX_PRICE_ARS;
  // Agregamos un margen del 20% para que el slider no quede "pegado" al producto más caro
  // y establecemos un piso mínimo de 100k para una mejor UX.
  const withMargin = value * 1.2;
  return Math.max(100000, Math.ceil(withMargin / 1000) * 1000);
};
