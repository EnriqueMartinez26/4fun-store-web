  ...
  return (
    <>
      {!isControlled && trigger && (
        <div onClick={() => setOpen(true)} className="w-full h-full cursor-pointer">
          {trigger}
        </div>
      )}

      <Dialog open={showOpen} onOpenChange={setShowOpen}>
        <DialogContent className="sm:max-w-[600px] rounded-search p-0 overflow-hidden gap-0 bg-card/95 backdrop-blur-xl border-white/10 shadow-3xl">
          <DialogHeader className="px-5 py-4 border-b border-white/5 bg-muted/20">
            ...
            <DialogDescription className="sr-only">
              Utilice este diálogo para buscar productos por nombre, género o plataforma en tiempo
              real.
            </DialogDescription>
            <form onSubmit={handleSearch} className="flex items-center gap-3">
              <Search className="h-5 w-5 shrink-0 text-primary opacity-80" />
              <input
                className="flex h-12 w-full rounded-search bg-transparent py-4 text-base font-medium text-white outline-none placeholder:text-muted-foreground placeholder:italic"
              ...
              />
              {loading && (
                <Loader2 className="h-5 w-5 animate-spin shrink-0 text-primary opacity-50" />
              )}
            </form>
          </DialogHeader>
          ...
          <div className="max-h-[450px] overflow-y-auto p-3 custom-scrollbar">
            {query.trim().length === 0 ? (
              ...
              <div className="py-12 text-center space-y-2">
                <p className="text-sm text-white font-semibold uppercase tracking-tight">
                  Sin resultados
                </p>
                <p className="text-xs text-muted-foreground">
                  No hay resultados registrados para: &quot;{query}&quot;
                </p>
              </div>
            ) : (
                ...
                {results.map((product) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-white group-hover:text-primary transition-colors truncate uppercase tracking-tight">
                        {product.name}
                      </p>
                    ...
                    <div className="text-right shrink-0">
                      {(product.discountPercentage ?? 0) > 0 ? (
                        <div className="flex flex-col items-end">
                          <span className="text-[10px] text-muted-foreground line-through decoration-red-500 font-mono italic opacity-90">
