  ...
  return (
    <div className="min-h-screen bg-background text-foreground pb-20 relative overflow-x-hidden">
      ...
      <main className="container mx-auto px-4 pt-10 max-w-7xl animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <Breadcrumbs
          items={[{ label: 'Productos', href: '/productos' }, { label: breadcrumbLabel }]}
        />
        ...
        <div className="mb-10">
          ...
          <div className="flex items-center gap-4">
            <Badge className="bg-primary/10 text-primary border-primary/20 font-semibold uppercase tracking-widest text-[10px]">
              Vendedor Verificado
            </Badge>
            ...
            <div className="space-y-6">
              ...
              <div className="prose prose-invert max-w-none text-muted-foreground leading-relaxed text-lg">
                <p className="whitespace-pre-line opacity-90">
                  {game.description ||
                    'Este producto no tiene descripcion disponible por el momento.'}
                </p>
              </div>
            </div>
              ...
              <Card className="bg-card/30 backdrop-blur-xl border-white/5 overflow-hidden">
                ...
                <CardContent className="pt-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4">
                    {game.requirements ? (
                      ...
                      <>
                        <SpecItem
                          ...
                          value={
                            game.specPreset?.toLowerCase() === 'high'
                              ? 'Ryzen 7 7700X / i7-13700K'
                              : game.specPreset?.toLowerCase() === 'mid'
                        ...
                        <SpecItem
                          label="Memoria RAM"
                          value={game.specPreset?.toLowerCase() === 'high' ? '32 GB DDR5' : '16 GB'}
                        />
                        <SpecItem
                          label="Gráficos (GPU)"
                          value={
                    ...
                    <SpecItem
                      label="Perfil de Hardware"
                      value={game.specPreset ?? undefined}
                      highlight
                    />
                  </div>
                </CardContent>
            ...
            <div className="sticky top-28 space-y-8 bg-card/40 backdrop-blur-2xl p-8 rounded-3xl border border-white/10 shadow-3xl ring-1 ring-white/5">
              ...
              <div className="space-y-6">
                {!isFree ? (
                  ...
                  <div className="text-5xl font-bold text-primary animate-pulse tracking-tighter">
                    FREE ACCESS
                  </div>
                )}
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <Button
                    ...
                    className={cn(
                      ...
                      isWishlisted &&
                        'bg-red-500 border-red-500 text-white hover:bg-red-600 hover:border-red-600 shadow-[0_0_20px_rgba(239,68,68,0.3)]'
                    )}
                  ...
                  {vm.canEdit && (
                    <Button
                      ...
                      variant="ghost"
                      size="lg"
                      className="w-full mt-4 hover:bg-primary/10 hover:text-primary font-bold transition-all duration-300"
                    >
                      <Link href={vm.editUrl}>EDITAR PRODUCTO</Link>
                    </Button>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-4">
                <MetaRow
                  icon={<Globe className="w-4 h-4 text-primary/60" />}
                  label="Desarrollador"
                  value={game.developer ?? undefined}
                />
                <MetaRow
                ...
                />
                <MetaRow
                  icon={<Gamepad2 className="w-4 h-4 text-primary/60" />}
                  label="Categoría"
                  value={game.genre?.name}
                />
                <MetaRow
                  icon={<Disc className="w-4 h-4 text-primary/60" />}
                  label="Distribución"
                  value="Licencia Digital"
                />
                <MetaRow
                  icon={<Info className="w-4 h-4 text-primary/60" />}
                  label="Lanzamiento"
                  value={
                    game.releaseDate
                      ? new Date(game.releaseDate).toLocaleDateString('es-AR')
                      : undefined
                  }
                />
              </div>
    ...
    <div className="flex flex-col gap-1 pb-2 md:pb-0">
      ...
      <span
        className={cn(
          ...
          highlight ? 'text-primary uppercase' : 'text-white'
        )}
      >
        {value}
      </span>
