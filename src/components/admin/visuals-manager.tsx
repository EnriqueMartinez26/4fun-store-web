...
export function VisualsManager() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      toast({
        variant: 'destructive',
        title: 'Error de Red',
        description: 'No se pudieron sincronizar las categorías.',
      });
    } finally {
      setLoading(false);
    }
  ...
  const getLabel = () => {
    switch (activeTab) {
      case 'platform':
        return 'Registrar Plataforma';
      case 'genre':
        return 'Registrar Género';
    }
  };
        ...
        <CardHeader className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <TabsList className="grid w-[400px] grid-cols-2 bg-muted/20 border border-white/5">
              <TabsTrigger value="platform" className="data-[state=active]:bg-primary">
                Plataformas
              </TabsTrigger>
              <TabsTrigger value="genre" className="data-[state=active]:bg-primary">
                Géneros
              </TabsTrigger>
            </TabsList>
          </div>
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <p className="text-sm text-muted-foreground">
              Gestion de imagenes y nombres para plataformas y generos.
            </p>
  ...
  const handleDelete = async (ids: string[]) => {
    ...
    try {
      ...
      toast({
        ...
        description: error.message || 'Error al procesar la baja.',
      });
    }
  };
      ...
      {selectedIds.size > 0 && (
        <div className="bg-destructive/10 border border-destructive/20 p-3 rounded-lg flex items-center justify-between animate-in fade-in slide-in-from-top-2">
          <span className="text-sm font-bold text-destructive">
            {selectedIds.size} seleccionados para eliminar
          </span>
          <Button
            variant="destructive"
            size="sm"
        ...
        <Table>
          <TableHeader className="bg-muted/30">
            <TableRow>
              ...
              <TableHead className="w-[120px] font-bold uppercase tracking-widest text-xs text-muted-foreground">
                Activo
              </TableHead>
              <TableHead className="font-bold uppercase tracking-widest text-xs text-muted-foreground">
                Nombre
              </TableHead>
              <TableHead className="text-right font-bold uppercase tracking-widest text-xs text-muted-foreground">
                Acciones
              </TableHead>
            </TableRow>
          </TableHeader>
            ...
            {items.map((item) => (
              <TableRow key={item.id} className="hover:bg-white/5 transition-colors border-white/5">
                ...
                <TableCell>
                  ...
                  <div className="text-xs font-mono text-muted-foreground uppercase opacity-70 tracking-tighter">
                    {item.id}
                  </div>
                </TableCell>
  ...
  const handleCreate = async () => {
    if (!id || !name) {
      toast({
        ...
        description: 'ID y Nombre son campos requeridos.',
      });
      return;
    }
      ...
      <DialogContent className="bg-card/95 backdrop-blur-xl border-white/10">
        ...
        <div className="grid gap-4 py-4">
          <div className="grid grid-cols-4 items-center gap-4">
            <Label
              htmlFor="new-id"
              className="text-right text-xs uppercase font-bold tracking-widest"
            >
              ID
            </Label>
            <Input
            ...
            <div className="col-span-3 space-y-3">
              <div className="relative h-24 w-full rounded-lg border border-dashed border-white/10 bg-muted/20 flex items-center justify-center overflow-hidden">
                {imageUrl ? (
                  ...
                  <span className="text-[10px] text-muted-foreground">SIN ACTIVO ASIGNADO</span>
                )}
              </div>
              <Input
                type="file"
...
function EditDialog({
      ...
      const loadDetails = async () => {
        try {
          ...
          toast({
            variant: 'destructive',
            title: 'Error',
            description: 'Fallo en hidratación de detalles.',
          });
          setOpen(false);
        } finally {
          setFetching(false);
  ...
  const handleSave = async () => {
    if (!newId || !name) {
      toast({ variant: 'destructive', title: 'Error', description: 'ID y Nombre obligatorios.' });
      return;
    }
    setLoading(true);
    try {
      ...
      toast({
        ...
        description: error.message || 'No se pudo salvar.',
      });
    } finally {
      setLoading(false);
    }
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="bg-card/95 backdrop-blur-xl border-white/10">
        <DialogHeader>
          <DialogTitle className="text-xl font-headline text-white">
            Refactorear {type === 'platform' ? 'Plataforma' : 'Género'}
          </DialogTitle>
        </DialogHeader>
        {fetching ? (
          ...
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
              <Label className="text-right text-xs uppercase font-bold tracking-widest text-muted-foreground">
                ID Técnico
              </Label>
              <Input
                value={newId}
                readOnly
                className="col-span-3 bg-background/20 border-white/5 text-muted-foreground cursor-not-allowed opacity-70"
              />
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <Label
                htmlFor="edit-name"
                className="text-right text-xs uppercase font-bold tracking-widest text-muted-foreground"
              >
                Nombre
              </Label>
              <Input
                ...
                value={name}
                onChange={(e) => setName(e.target.value)}
                className="col-span-3 bg-background/50 border-white/10 text-white"
              />
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <Label className="text-right text-xs uppercase font-bold tracking-widest text-muted-foreground">
        ...
        <DialogFooter className="flex justify-center sm:justify-center">
          <Button
            onClick={handleSave}
            disabled={loading || isUploading || fetching}
            className="w-fit px-10 font-bold"
          >
