...
export function AppSidebar() {
  ...
  return (
    <Sidebar
      collapsible="icon"
      className="z-40 border-r border-white/5 bg-card/95 backdrop-blur-xl"
    >
      <SidebarHeader>
        <SidebarMenu>
          <SidebarMenuItem>
            <SidebarMenuButton
              size="lg"
              asChild
              className="hover:bg-white/5 hover:text-white transition-colors cursor-pointer"
            >
              <Link href={user?.role === 'ADMIN' ? '/admin' : '/seller/dashboard'}>
                ...
                <div className="grid flex-1 text-left text-sm leading-tight group-data-[collapsible=icon]:hidden">
                  <span className="truncate font-headline font-bold text-white uppercase tracking-tight">
                    4Fun Dashboard
                  </span>
                  <span className="truncate text-[10px] lg:text-xs text-muted-foreground uppercase font-bold tracking-widest">
                    Panel de {user?.role === 'ADMIN' ? 'Admin' : 'Ventas'}
                  </span>
                </div>
              </Link>
            </SidebarMenuButton>
                ...
                <SidebarMenuItem key={item.title}>
                  ...
                  >
                    <Link href={item.url} className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-bold uppercase tracking-wide group-data-[collapsible=icon]:hidden">
                        {item.title}
                      </span>
                    </Link>
                  </SidebarMenuButton>
          ...
          <SidebarMenuItem>
            ...
            >
              <Avatar className="h-8 w-8 rounded-lg ring-1 ring-white/10 group-hover/logout:ring-destructive/30 transition-colors">
                ...
                <AvatarFallback className="rounded-lg bg-primary/10 text-primary font-bold text-xs lg:text-sm group-hover/logout:bg-destructive/10 group-hover/logout:text-destructive transition-colors">
                  {user?.name?.[0] || 'A'}
                </AvatarFallback>
              </Avatar>
              <div className="grid flex-1 text-left text-xs leading-tight min-w-0 group-data-[collapsible=icon]:hidden">
                <span className="truncate font-bold text-white group-hover/logout:text-destructive transition-colors">
                  {user?.name}
                </span>
                <span className="truncate text-[9px] lg:text-[10px] text-muted-foreground uppercase opacity-70 italic">
                  {user?.role}
                </span>
              </div>
