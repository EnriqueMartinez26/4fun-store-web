...
function SuccessContent() {
  ...
  const paymentLink =
    safeQueryValue(searchParams.get('payment_link')) || 'https://link.mercadopago.com.ar/4funstore';
  ...
  return (
    <Card className="w-full max-w-2xl border-none bg-card/40 backdrop-blur-3xl shadow-3xl rounded-[4px] overflow-hidden ring-1 ring-white/10">
      <CardHeader className="pt-12 pb-6 text-center bg-blue-500/5 border-b border-white/5">
        ...
        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-blue-400/80 mt-2">
          Pendiente de Liquidación Manual
        </p>
      </CardHeader>
      ...
      <CardContent className="space-y-8 px-10 pb-12 pt-8">
        <div className="space-y-4 text-center">
          <p className="text-white text-lg font-medium leading-relaxed">
            Tu reserva ha sido guardada exitosamente bajo el código de orden{' '}
            <span className="text-primary font-bold">{orderId ? `#${orderId}` : ''}</span>.
          </p>
          <p className="text-muted-foreground text-sm">
            Para finalizar la compra y que despachemos tus productos o claves digitales, necesitas
            completar el pago a través de nuestro enlace oficial de Mercado Pago.
          </p>
        </div>
        ...
        <div className="bg-[#009EE3]/10 border border-[#009EE3]/20 rounded-3xl p-8 flex flex-col items-center text-center space-y-6 relative overflow-hidden">
          ...
          <div className="relative z-10 space-y-2">
            ...
            <p className="text-[#009EE3] text-xs font-bold uppercase tracking-widest">
              Plataforma Segura
            </p>
          </div>
        ...
        <div className="bg-orange-500/10 border border-orange-500/20 p-5 rounded-2xl flex items-start gap-4">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-orange-200/70 leading-relaxed">
              Tras realizar el pago, nuestro personal administrativo validará el ingreso y marcará
              tu orden como &quot;Pagada&quot;. Puede tomar unos minutos en reflejarse en tu cuenta.
            </p>
          </div>
        </div>
        ...
        <div className="pt-6 border-t border-white/10 flex flex-col sm:flex-row justify-center gap-4">
          <Button
            asChild
            size="lg"
            className="h-14 bg-white/5 hover:bg-white/10 text-white border border-white/10 font-black uppercase text-[10px] tracking-widest transition-all rounded-xl"
          >
            <Link href="/account">
          ...
          <Button
            asChild
            size="lg"
            className="h-14 bg-primary text-white font-bold uppercase text-[10px] tracking-widest transition-all rounded-xl shadow-xl shadow-primary/20"
          >
            <Link href="/productos">
