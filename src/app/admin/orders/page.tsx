...
export default function AdminOrdersPage() {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      ...
      toast({
        ...
        description: 'No se pudo sincronizar el historial transaccional.',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const handleStatusChange = async (orderId: string, newStatus: string) => {
    try {
      ...
      toast({
        ...
        description: `Orden transicionada a: ${newStatus.toUpperCase()}`,
      });
      fetchOrders();
    } catch (error) {
      toast({
        title: 'Error Operativo',
        description: 'No se pudo mutar el registro de la operación.',
        variant: 'destructive',
      });
    }
  };
  ...
  const handleMarkAsPaid = async (orderId: string) => {
    try {
      ...
      toast({
        ...
        description:
          'Orden marcada como pagada y claves digitales enviadas por correo cuando corresponda.',
      });
      fetchOrders();
    } catch (error) {
      toast({
  ...
  const filteredOrders = searchTerm
    ? orders.filter((o) => {
        ...
        return (
          o.getDisplayId().toLowerCase().includes(term) ||
        ...
        <CardHeader className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 border-b border-white/5 pb-8">
          ...
          <div className="flex flex-wrap gap-2 w-full md:w-auto">
            <ExportReportDialog
              ...
              description="Formato de Salida Registral"
            />
          </div>
        </CardHeader>
              ...
              <TableBody>
                {loading ? (
                  ...
                  <TableRow>
                    ...
                    >
                      No se hallaron coincidencias transaccionales.
                    </TableCell>
                  </TableRow>
                ) : (
                    ...
                    >
                      ...
                      <TableCell>
                        <Badge
                          variant="outline"
                          className={cn(
                            'text-xs font-black uppercase py-0.5 px-3',
                            o.isDelivered() || o.isShipped()
                      ...
                      <TableCell>
                        <Badge
                          variant="outline"
                          className={cn(
                            'text-xs font-bold py-1 px-4 uppercase',
                            o.isPaid
                        ...
                        <DropdownMenu>
                          ...
                          >
                            ...
                            <DropdownMenuItem
                              ...
                              className="text-xs font-bold hover:bg-primary/10 hover:text-primary cursor-pointer"
                            >
                            ...
                            <DropdownMenuItem
                              onClick={() =>
                                toast({
                                  ...
                                  description:
                                    'Vista de detalle en desarrollo para auditoría profunda.',
                                })
                              }
                              className="text-xs font-bold hover:bg-primary/10 hover:text-primary cursor-pointer"
                            >
