...
const contactSchema = z.object({
  ...
  message: z
    .string()
    .min(10, 'Contanos un poco más sobre tu consulta (mínimo 10 letras)')
    .max(1000, 'El mensaje es demasiado largo (máximo 1000 letras)'),
});
  ...
  const onSubmit = async (data: ContactFormData) => {
    try {
      ...
      toast({
        ...
        description:
          error.message || 'No se pudo sincronizar la consulta con el servidor de soporte.',
        variant: 'destructive',
      });
    }
  };
  ...
  if (sent) {
    return (
      <div className="container mx-auto max-w-2xl py-24 px-4 animate-in fade-in zoom-in-95 duration-700">
        <Card className="text-center border-none bg-card/40 backdrop-blur-3xl shadow-3xl rounded-[4px] overflow-hidden">
          <CardContent className="pt-16 pb-12 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-muted-foreground max-w-md mx-auto leading-relaxed">
                Su solicitud de contacto ha sido normalizada e ingresada. Un operador técnico
                responderá vía correo electrónico en breve.
              </p>
            </div>
            <Button
              variant="outline"
              onClick={() => setSent(false)}
              className="mt-6 h-12 px-8 border-white/10 text-white font-black uppercase text-[10px] tracking-widest hover:bg-white/5 rounded-xl transition-all"
            >
        ...
        <CardHeader className="pt-12 px-10">
          <div className="flex items-center gap-4 mb-3">
            ...
            <div>
              ...
              <CardDescription className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground opacity-70">
                Soporte y Asesoría
              </CardDescription>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="space-y-3">
                <Label
                  htmlFor="firstName"
                  className="text-xs font-black uppercase tracking-widest text-muted-foreground ml-1 flex items-center gap-2"
                >
                ...
                <Input
                  id="firstName"
                  {...register('firstName')}
                  className="h-12 bg-white/5 border-white/10 rounded-2xl focus:ring-primary/40 text-white text-base"
                />
                {errors.firstName && (
            ...
            <div className="space-y-3">
              ...
              <Textarea
                id="message"
                className="min-h-[160px] bg-white/5 border-white/10 rounded-[4px] focus:ring-primary/40 text-white text-base resize-none p-6"
                {...register('message')}
              />
              {errors.message && (
                <p className="text-[10px] font-bold text-destructive uppercase tracking-widest mt-1 ml-1">
            ...
            <div className="flex justify-end pt-4">
              <Button
                ...
                className="w-full sm:w-auto h-14 px-12 bg-white/5 text-white hover:bg-primary/10 hover:text-primary border border-white/10 hover:border-primary font-black uppercase text-xs tracking-[0.15em] rounded-2xl shadow-xl hover:shadow-primary/20 transition-all duration-300 group"
              >
                {isSubmitting ? (
                  <>
