...
export default function SellerProductsPage() {
    ...
    async (page = 1, searchQuery = '') => {
      try {
        ...
        const { products: fetchedProducts, meta: fetchedMeta } =
          await ProductApiService.getSellerProducts({ page, limit: 10, search: searchQuery });
        ...
        toast({
          ...
          description: 'No se pudieron obtener tus productos.',
        });
      } finally {
        setLoading(false);
      }
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!confirm(`¿Estás seguro de quitar "${name}" de la tienda? Esta acción es irreversible.`))
      return;
    try {
      ...
      toast({
        variant: 'destructive',
        title: 'Error',
        description: 'No posees permisos para eliminar este activo.',
      });
    }
  };
    ...
    <div className="container mx-auto p-6 lg:p-10 space-y-8 animate-in fade-in duration-700">
      ...
      <Card className="border-none bg-card/40 backdrop-blur-3xl shadow-2xl rounded-[4px] overflow-hidden">
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            ...
            <Button asChild className="px-8">
              <Link href="/seller/products/new">
                <Plus className="mr-2 h-5 w-5" /> Crear Publicación
              </Link>
            </Button>
          </div>
        </CardHeader>
        ...
        <CardContent className="p-10">
          ...
          <div className="rounded-[4px] border border-white/5 overflow-hidden ring-1 ring-white/5 bg-black/20">
            <Table>
              <TableHeader className="bg-white/5">
                <TableRow className="hover:bg-transparent border-white/5">
                  <TableHead className="font-black uppercase tracking-widest text-[10px] text-muted-foreground p-6">
                    Info del Juego
                  </TableHead>
                  ...
                  <TableHead className="font-black uppercase tracking-widest text-[10px] text-muted-foreground text-right p-6">
                    Gestión
                  </TableHead>
                </TableRow>
              </TableHeader>
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell>
                          ...
                          {p.isOnDiscount() && (
                            <Badge className="bg-green-500/10 text-green-400 border-none text-[8px] font-black mt-1">
                              OFERTA {p.getDiscountBadge()}
                            </Badge>
                          )}
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell className="text-right p-6">
                          <div className="flex justify-end gap-3 opacity-0 group-hover:opacity-100 transition-all translate-x-4 group-hover:translate-x-0">
                            <Button variant="ghost" size="icon" asChild>
                              <Link href={`/seller/products/${p.getId()}`}>
