...
export default function WishlistPage() {
  ...
  return (
    <div className="container mx-auto max-w-screen-2xl px-4 py-16 md:py-24 animate-in fade-in duration-1000">
      ...
      {wishlist.length === 0 ? (
        <div className="text-center py-24 bg-card/10 backdrop-blur-md rounded-[4px] border-2 border-dashed border-white/5 animate-in zoom-in-95 duration-700">
          ...
          <p className="mt-4 text-muted-foreground max-w-sm mx-auto leading-relaxed text-sm font-medium">
            Guarda tus juegos favoritos para encontrarlos rapido. Explora el catalogo y agregalos
            con un clic.
          </p>
          <Button
            asChild
            variant="outline"
            className="mt-10 h-14 px-10 rounded-xl font-black uppercase tracking-widest text-[10px] bg-white/5 text-white hover:bg-primary/10 hover:text-primary border border-white/10 hover:border-primary transition-all duration-500 shadow-xl hover:shadow-primary/20"
          >
            <Link href="/productos">
              Explorar el Catálogo <ArrowRight className="ml-2 h-4 w-4" />
            </Link>
          </Button>
        </div>
      ) : (
      ...
      {wishlist.length > 0 && (
        <div className="mt-16 text-center">
          <Link
            href="/productos"
            className="inline-flex items-center gap-2 text-[10px] font-black text-muted-foreground uppercase tracking-[0.4em] hover:text-white transition-all group"
          >
            <ShoppingBag className="h-4 w-4 group-hover:-translate-y-1 transition-transform" />{' '}
            Seguir explorando juegos
          </Link>
        </div>
      )}
    </div>
