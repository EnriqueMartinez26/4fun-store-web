    ...
    const verify = async () => {
      try {
        ...
        if (response.success) {
          ...
          setStatus('error');
          setMessage(
            response.message || 'El token ha expirado o ya ha sido procesado por el sistema.'
          );
        }
      } catch (error: any) {
    ...
    <div className="container flex items-center justify-center min-h-[85vh] px-4 animate-in fade-in zoom-in-95 duration-700">
      <Card className="w-full max-w-md border-none bg-card/40 backdrop-blur-3xl shadow-3xl rounded-[4px] overflow-hidden ring-1 ring-white/10">
        <CardHeader className="pt-12 pb-6 text-center">
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground opacity-60 mt-2">
            Verificación de Credenciales en el Padrón
          </p>
        </CardHeader>
            ...
            <div className="animate-in slide-in-from-bottom-4 duration-500 w-full flex flex-col items-center gap-6">
              ...
              <div className="text-center space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  Sus credenciales han sido normalizadas. La cuenta ahora posee privilegios de
                  acceso completos.
                </p>
              </div>
              <Button
                asChild
                className="w-full h-14 bg-primary text-white font-bold uppercase text-[10px] tracking-widest rounded-xl shadow-xl transition-all mt-4"
              >
                <Link href="/login">
            ...
            <div className="animate-in slide-in-from-bottom-4 duration-500 w-full flex flex-col items-center gap-6">
              ...
              <div className="text-center space-y-2">
                ...
                <p className="text-sm text-muted-foreground pt-3">
                  No se pudo constatar la validez de la solicitud. Por favor, genere un nuevo
                  identificador desde su perfil.
                </p>
              </div>
              <Button
                asChild
                variant="outline"
                className="w-full h-12 border-white/10 text-white font-black uppercase text-[10px] tracking-widest rounded-xl hover:bg-white/5 transition-all mt-4"
              >
      ...
      <div className="fixed bottom-10 flex flex-col items-center gap-2 opacity-30 select-none">
        ...
        <span className="text-[9px] font-black uppercase tracking-[0.4em]">
          Email Verification Module v2.0
        </span>
      </div>
    </div>
  );
