  ...
  if (compareList.length === 0) {
    return (
      <div className="container mx-auto py-32 text-center animate-in fade-in zoom-in-95 duration-700 px-4">
        ...
        <h1 className="text-4xl font-headline font-bold mb-4 text-white tracking-tight">
          Comparar juegos
        </h1>
        <p className="text-muted-foreground mb-8 text-sm uppercase tracking-widest font-black opacity-60">
          Todavia no agregaste juegos para comparar.
        </p>
        <Button
          asChild
          className="h-12 px-8 rounded-full font-bold uppercase tracking-widest text-[10px] bg-primary text-white hover:bg-primary/90 shadow-xl transition-all"
        >
  ...
  return (
    <div className="container mx-auto max-w-screen-2xl px-4 py-16 animate-in fade-in duration-1000">
      <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-12 border-b border-white/5 pb-8">
        <div className="space-y-1">
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground opacity-70">
            Comparando {compareList.length} juegos en paralelo
          </p>
        </div>
        <Button
          variant="outline"
        ...
        <Table className="min-w-[900px]">
          <TableHeader className="bg-muted/30">
            <TableRow className="border-white/5">
              <TableHead className="w-[250px] font-black uppercase tracking-[0.2em] text-[10px] text-muted-foreground pl-10 h-20">
                Ficha Técnica
              </TableHead>
                ...
                <TableHead key={game.id} className="min-w-[280px] relative h-20">
                  <div className="flex items-center justify-between gap-4 pr-4">
                    <Link href={`/productos/${game.id}`} className="flex-1">
                      <span className="font-bold text-white text-sm hover:text-primary transition-colors block truncate">
                        {game.name}
                      </span>
                    </Link>
                    <Button
          ...
          <TableBody>
            ...
            <TableRow className="border-white/5 hover:bg-white/5 transition-colors">
              <TableCell className="font-black uppercase tracking-widest text-[10px] text-muted-foreground pl-10 py-6">
                Inversión (ARS)
              </TableCell>
              {compareList.map((game) => (
            ...
            <TableRow className="border-white/5 hover:bg-white/5 transition-colors">
              ...
              {compareList.map((game) => (
                <TableCell
                  key={game.id}
                  className="text-center text-sm font-medium text-white/80 opacity-60"
                >
            ...
            <TableRow className="border-transparent hover:bg-white/5 transition-colors h-24">
              ...
              {compareList.map((game) => (
                <TableCell key={game.id} className="text-center">
                  <Button
                    asChild
                    className="w-[80%] h-11 bg-white/5 hover:bg-primary/10 hover:text-primary border border-white/10 hover:border-primary transition-all font-black uppercase text-[10px] tracking-widest rounded-xl shadow-xl"
                  >
      ...
      <div className="mt-8 flex justify-center">
        <Link
          href="/productos"
          className="flex items-center gap-2 text-muted-foreground hover:text-white transition-colors text-xs font-black uppercase tracking-[0.2em]"
        >
